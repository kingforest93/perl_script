...
cds2aa.pl  --  translate nucleic acid sequence to amino acid sequence
...
(1) translate coding DNA sequence to protein seqeunce. 
	...
	The stop codon will be translated into "*".
	The unknow codons will be translated into "X" ,which are often caused by gaps, such as "NNN", or partial gaps "AGN".
...
Note that before translating cds into protein sequence, it is neccessary to check the completeness.
...
=head1 Command-line Option
  ...
  --check             check the completeness of gene model
  --stop_asterisk     translate stop codon into asterisk
  --verbose           output verbose information to screen  
  --help              output help information to screen  

=head1 Usage Exmples
  (1)translate whole cds sequence
  perl ./cds2aa.pl test-data/BMF000017.cds.fa 
  (2)translate individual exon by phase information
  perl ./cds2aa.pl test-data/BMF000017.exon.fa 
  (3)check whether the cds sequence is complete
  perl ./cds2aa.pl -check test-data/test_3chrs.fa.fgenesh.100.cds.fa 
  (4)keep the product of stop codon as asterisk
  perl ./cds2aa.pl --stop_asterisk test-data/BMF000017.cds.fa 
=cut
...
my ($Check,$Trans_table,$Stop_asterisk, $Verbose,$Help);
GetOptions(
	"translate:s"=>\$Trans_table,
	"check"=>\$Check,
	"stop_asterisk"=>\$Stop_asterisk,
	"verbose"=>\$Verbose,
	"help"=>\$Help
...
sub cds2aa {
	...
	##replace degenerate bases (two-allele SNPs) into one bases
	##R: A/G;  Y: C/T;  M: A/C;  K: G/T;  S: G/C;  W: A/T;
	$seq =~ tr/RYMKSW/ACAGGA/;
